...
public class THashMap<K, V> extends TObjectHash<K> implements Map<K, V>, Externalizable {
    ...
    public V put( K key, V value ) {
        ...
        _values[index] = value;
        if ( isNewMapping ) {
            // Since insertIndex() did override the old value we
            // can not know whether a FREE slot was used or a REMOVED
            // one. We still report an extra FREE slot as used. We may compact
            // a bit too early as a result but then again using a REMOVED
            // slot means then number of FREE slot needs to increase
            //
            // TODO: review the above reasoning
            postInsertHook( true );
        }
